# Fish Generator
![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)  

Version `0.0.0.3`

**[Tester la version déployée sur GitHub Pages](https://laz-r.github.io/fish-generator/)**

## Présentation
Générateur de poissons tropicaux

---

This is mostly a *proof of concept* so the code here is awfull, I know.

## Changelog

- v 0.0.0.3 - rework generation oeil + ajout page trade et rework export fishier + passage du type d'import en "application/octet-stream" + filtres galerie + rareté morphologique légère + ajout chiffres page d'accueil
- v 0.0.0.2 - remise au format app + ajout shiny + legendaire + ajout galerie + ajout export et trade
- v 0.0.0.1 - init

---

Copyright ©2026 LAZ-R - Tous droits réservés